...
export default async function handler(req, res) {
  ...
  var owner_address = req.body["address"]
  ...
  console.log(`Start minting to ${owner_address}`)
  try {
    const tx = await contract.methods.mint(owner_address);
    ...
    contract.methods.getByTokenId(token).call(
      function (err, res_get) {
        if (err) {
          console.log("An error occurred", err)
          res.status(500).json({
            message: `Avatar minted with token: ${token}
            But failed to get detail: ${err}`
          })
        }
        var avatar = {
          id: token,
